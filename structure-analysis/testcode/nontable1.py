...
for p_num, p in pages.items():
    ...
    # imgfilebasename=p['image'].split('/')[1]
    # DATAPATH=p['image'].split('/')[0]
    imgfilebasename = 'demo.png'
    DATAPATH = '../../docs/'
    ...
    # detect the lines
    lines_hough = iproc_obj.detect_lines_inv(canny_low_thresh=50, canny_high_thresh=150, canny_kernel_size=3,
                                         hough_rho_res=1,
    ...
    img_vert_lines_file=iproc_obj.draw_lines_filtered_straight(imgproc.DIRECTION_VERTICAL)
...
pttrn_table_row_beginning = re.compile(
    r'^[\d Oo][\d Oo]{2,} +[A-ZÄÖÜ]')  # a (possibly malformed) population number + space + start of city name
...
for p_num, p in pages.items():
    ...
    img_grid = iproc_obj.draw_grid(col_positions_noscale[p_num], map(lambda x:x*page_scaling_y,line_positions[p_num]))
