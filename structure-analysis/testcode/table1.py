...
for p_num, p in pages.items():
    ...
    img_vert_lines_file=iproc_obj.draw_lines_usinginv(imgproc.DIRECTION_VERTICAL)
    save_img_file = os.path.join(OUTPUTPATH, '%sfiltered-vertical-lines.png' % imgfilebasename)
...
for p_num, p in pages.items():
    ...
    img_grid = iproc_obj.draw_grid(col_positions_noscale[p_num], line_positions[p_num])
    n_rows = len(grid)
    n_cols = len(grid[0])
