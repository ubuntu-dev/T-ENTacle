...
class ImageProc:
    ...
    def __init__(self, imgfile, input_img=[]):
        """
    ...
    def detect_lines(self, canny_low_thresh, canny_high_thresh, canny_kernel_size,
        ...
        cv2.imwrite("docs/image_edges.png", self.edges)  # todo: delete
    ...
    def detect_lines_inv(self, canny_low_thresh, canny_high_thresh, canny_kernel_size,
                     hough_rho_res, hough_theta_res, hough_votes_thresh,
                     gray_conversion=cv2.COLOR_BGR2GRAY):
        """
        Detect lines in input image using hough transform.
        ...
        for rho, theta, theta_norm, line_dir in self.lines_hough:
            if line_dir == DIRECTION_VERTICAL and (only_direction is None or only_direction == DIRECTION_VERTICAL):
                ...
                vert_deviations.append(-deviation)
                    ...
                    only_direction is None or only_direction == DIRECTION_HORIZONTAL):
    ...
    def find_clusters(self, direction, method, **kwargs):
        ...
        for i, (p1, p2, line_dir) in enumerate(lines_ab):
            if line_dir == direction:
                if direction==DIRECTION_VERTICAL and  abs(int(p1[0]) - int(p2[0])) < 50 :
                    lines_ab_new.append((p1, p2, line_dir))
                elif direction==DIRECTION_HORIZONTAL and  abs(int(p1[1]) - int(p2[1])) < 50 :
                    lines_ab_new.append((p1, p2, line_dir))


        lines_ab = lines_ab_new

        # todo: straighten the lines (take cue for clustering). Find the continuous pixels above threshold
        ...
        if remove_empty_cluster_sections_use_texts is not None:
            ...
            clusters_w_vals_and_centers = [
                (ind, vals, remove_empty_cluster_sections_clust_center_fn(vals) / remove_empty_cluster_sections_scaling)
    ...
    def draw_lines_filtered_straight(self, direction):

        lines_ab = self.ab_lines_from_hough_lines(self.lines_hough)

        baseimg = self._baseimg_for_drawing(True)
        ...
        for i, (p1, p2, line_dir) in enumerate(lines_ab):
            if line_dir==direction:
                ...
                if direction == DIRECTION_HORIZONTAL and abs(int(p1[1]) - int(p2[1])) < 50:
                    line_color = (0, 255, 0)
                    cv2.line(baseimg, pt_to_tuple(p1), pt_to_tuple(p2), line_color, self.DRAW_LINE_WIDTH)

        return baseimg

    def draw_grid(self,col_positions, row_positions):
        baseimg = self._baseimg_for_drawing(True)
        self.draw_lines_in_dir(baseimg, DIRECTION_VERTICAL, col_positions, (225, 225, 0))
        self.draw_lines_in_dir(baseimg, DIRECTION_HORIZONTAL, row_positions, (225, 225, 0))
        return baseimg
